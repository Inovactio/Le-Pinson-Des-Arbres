...
public class Controller {
    ...
    @FXML
    private Button connect, createLobby, showLobbies, playGame, refreshButton, validateButton, joinToMainMenuButton, lobbyToMainMenuButton, ipConnectionButton;
    @FXML
    private Button incrNbRounds, decrNbRounds, incrNbImpostors, decrNbImpostors, incrTurnTime, decrTurnTime;
    @FXML
    private TextField usernameTextField, ipSelectionTextField;
    @FXML
    ...
    @FXML
    private void connectToServer(ActionEvent event) throws Exception {
        client.initConnection(ipSelectionTextField.getText());
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/usernameSelection.fxml"));
        loader.setController(client.getController());
        Parent root = loader.load();
        Stage stage = (Stage) ipConnectionButton.getScene().getWindow();
        Scene scene = new Scene(root, 1080, 720);
        stage.setScene(scene);
        stage.show();
